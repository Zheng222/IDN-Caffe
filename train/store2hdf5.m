function [curr_dat_sz, curr_lab_sz,curr_bic_sz] = store2hdf5(filename, data, labels, bic, create, startloc, chunksz)  
  ...
  if create
    ...
    h5create(filename, '/bic', [bic_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [lab_dims(1:end-1) chunksz]);
    if ~exist('startloc','var') 
      startloc.dat=[ones(1,length(dat_dims)-1), 1];
      startloc.lab=[ones(1,length(lab_dims)-1), 1];
      startloc.bic=[ones(1,length(bic_dims)-1),1];
    end 
  else  % append mode
    if ~exist('startloc','var')
      ...
      prev_bic_sz=info.Datasets(3).Dataspace.Size;
      ...
      assert(prev_bic_sz(1:end-1)==lab_dims(1:end-1), 'Bic dimensions must match existing dimensions in dataset');
      startloc.dat=[ones(1,length(dat_dims)-1), prev_dat_sz(end)+1];
      startloc.lab=[ones(1,length(lab_dims)-1), prev_lab_sz(end)+1];
      startloc.bic=[ones(1,length(lab_dims)-1),prev_bic_sz(end)+1];
    end
  end
  ...
  if ~isempty(data)
    ...
    h5write(filename, '/bic', single(bic),startloc.bic,size(bic));
  end
  ...
  if nargout
    ...
    curr_bic_sz=info.Datasets(3).Dataspace.Size;
  end
end
